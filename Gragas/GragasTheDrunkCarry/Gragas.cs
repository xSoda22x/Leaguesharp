...
namespace GragasTheDrunkCarry
{
    class Gragas
    {
        ...
        static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Config.SubMenu("Draw").AddItem(new MenuItem("Draw R", "Draw R Range").SetValue(true));
            Config.AddItem(
    new MenuItem("Insec", "Insec").SetValue(new KeyBind("A".ToCharArray()[0], KeyBindType.Press)));
        ...
        private void SmartKs()
        {
            ...
            foreach (Obj_AI_Hero target in ObjectManager.Get<Obj_AI_Hero>().Where(x => x.IsValidTarget(700) && !x.HasBuffOfType(BuffType.Invulnerability)).OrderByDescending(GetCDamage))
            {
                if (target != null)
                {
                    //R
